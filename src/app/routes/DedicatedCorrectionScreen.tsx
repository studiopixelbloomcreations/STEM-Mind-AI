...
export const DedicatedCorrectionScreen: React.FC = () => {
  ...
  const question = state.question || {
    ...
    hint: 'Check your signs and units',
    howToApproach: 'Carefully substitute the values and follow order of operations.',
    difficulty: 'medium',
    teachingSteps: [
      {
        stepNumber: 1,
        title: 'Diagnose the Calculation Slip',
        visual: `<div style="padding:16px; border:1px solid #f87171; border-radius:12px; text-align:center; font-family:monospace; background:rgba(248,113,113,0.06);"><strong>Notice where the derivation drifted:</strong> Double-check the sign convention.</div>`,
        speech: 'It is completely okay to make a mistake — that is actually where the deepest learning happens. Let us see why your choice was tempting and where the calculation differed.',
      },
      {
        stepNumber: 2,
        title: 'Apply the Correct Governing Step',
        visual: `<div style="padding:16px; border:1px solid #38bdf8; border-radius:12px; text-align:center; font-family:monospace; background:rgba(56,189,248,0.06);"><strong>Correct Step:</strong> Substitute values with explicit brackets to preserve signs.</div>`,
        speech: 'When we substitute our numbers into the formula, remember to carry our signs and units through each line carefully.',
      },
      {
        stepNumber: 3,
        title: 'Arrive at the True Solution',
        visual: `<div style="padding:16px; border:1px solid #34d399; border-radius:12px; text-align:center; font-family:monospace; background:rgba(52,211,153,0.06);"><strong>Final Answer:</strong> Result matches verified syllabus key.</div>`,
        speech: 'Now we simplify cleanly. Notice how we reach the verified solution! You will definitely master this on the next try.',
      },
    ],
  };
    ...
    : [
        {
          ...
          speech: 'Let us examine how to solve this step by step.',
        },
      ];

  const userAnswer = state.userAnswer || 'Submitted Choice';
  ...
  // Play narration for current correction step and automatically advance when speech completes
  useEffect(() => {
      ...
      voiceSynthesizer.speak(
        ...
        () => {
          ...
          if (currentStepIndex < steps.length - 1) {
            ...
            }, 1200); // Friendly pause between steps
          } else {
  ...
  return (
    <div className="min-h-screen w-full bg-[var(--color-bg-base)] text-[var(--color-text-primary)] flex flex-col p-6 lg:p-12 relative select-none font-body">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg font-display font-extrabold text-[var(--color-text-primary)]">
                  Answer Correction &amp; Repair
                </h1>
                <span className="px-2 py-0.5 rounded-full bg-[var(--color-warning)]/15 text-[var(--color-warning)] font-mono text-[10px] font-bold uppercase border border-[var(--color-warning)]/30">
                  Step Breakdown
                </span>
              </div>
      ...
      <main className="max-w-5xl w-full mx-auto flex-1 flex flex-col justify-between space-y-6">
        {/* Answer Contrast Card (Submitted vs Correct) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="p-5 bg-[var(--color-bg-surface)] border border-[var(--color-danger)]/40 rounded-xl">
            <div className="flex items-center gap-2 mb-2 text-xs font-mono text-[var(--color-danger)] font-bold uppercase">
              <Icon icon={XCircle} size={15} />
              <span>Your Answer Choice</span>
            </div>
            <p className="text-base font-medium text-[var(--color-text-primary)] font-mono line-clamp-2">
              {userAnswer}
            </p>
          </Card>

          <Card className="p-5 bg-[var(--color-bg-surface)] border border-[var(--color-success)]/40 rounded-xl">
            <div className="flex items-center gap-2 mb-2 text-xs font-mono text-[var(--color-success)] font-bold uppercase">
        ...
        <Card className="p-5 bg-[var(--color-bg-surface)] border border-[var(--color-border)] rounded-xl">
          <span className="text-xs font-mono text-[var(--color-text-secondary)] block mb-1">
            Question Context
          </span>
          <h3 className="text-base sm:text-lg font-display font-bold text-[var(--color-text-primary)] leading-snug">
            {question.question}
          </h3>
        </Card>
