 ...
 * 
 * Strict Phase 11 Mandate: Zero fake-content fallbacks. If all candidate AI models fail,
 * the error is propagated to the caller to display an honest failure state with Retry.
 */
export async function generateFullSessionConcurrently(
  ...
  const generateSingleWorker = async (questionIndex: number): Promise<SessionQuestion> => {
    try {
      ...
      // Section A1: Never substitute fake content on total failure; propagate real error
      throw err;
    }
  };
...
export async function generateQuestionFromCouncil(
  ...
  // Section A1: Propagate genuine error rather than inventing fake content
