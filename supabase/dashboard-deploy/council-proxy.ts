 ...
 * Project: jxhljizbivkrnpzwswce | Function slug must match filename: council-proxy
 * Deploys the universal Gemini proxy holding GEMINI_API_KEY server-side.
 */
  ...
  try {
    ...
    if (action === 'generateContent') {
      ...
      try {
        ...
        return jsonWithCors(
          request,
          { error: aborted ? `Model ${model} timed out after ${AGENT_TIMEOUT_MS}ms` : e?.message || 'Proxy request failed' },
          aborted ? 504 : 502
        );
      } finally {
        clearTimeout(timer);
    ...
    // --- Action 3: tts (Gemini Speech Synthesis) ---
    if (action === 'tts') {
      const text = String(body.text || '');
      const voice = String(body.voice || 'Kore');
      const ttsModel = String(body.model || 'gemini-3.1-flash-tts').replace(/^models\//, '');

      if (!text) return jsonWithCors(request, { error: 'Missing text parameter' }, 400);

      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), AGENT_TIMEOUT_MS);
      try {
        const res = await fetch(
          `${GEMINI_BASE}/models/${encodeURIComponent(ttsModel)}:generateContent?key=${env.geminiKey}`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        const data = await res.json();
        if (!res.ok) return jsonWithCors(request, data, res.status);
        return jsonWithCors(request, data);
      } catch (e: any) {
        const aborted = e instanceof Error && e.name === 'AbortError';
        return jsonWithCors(request, { error: aborted ? 'TTS request timed out' : e.message }, aborted ? 504 : 502);
      } finally {
        clearTimeout(timer);
      }
    }

    // --- Action 4: Individual Council Agent execution ---
    const agentId = String(body.agent || '');
    const agentDef = AGENTS[agentId];
    if (!agentDef) {
      return jsonWithCors(request, { error: `unknown agent: ${agentId}` }, 400);
    }

    const [system, temperature] = agentDef;
    const userPayload = (body.payload as Record<string, unknown>) || {};
    const userText = buildUserTurn(agentId, userPayload);

    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), AGENT_TIMEOUT_MS);
    try {
      const res = await fetch(
        `${GEMINI_BASE}/models/${encodeURIComponent(GEMINI_MODEL)}:generateContent?key=${env.geminiKey}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
